...
avoid_adjacent: []
requires_pair: null
blocked_by: "Criterion 2 is unrun, and criterion 3 has two renders, one a fail by this type's own definition. Criterion 1 is cleared: 10 distinct sources on 2026-09-15 (validate.py --evidence)."
---
...
## BLOCK
**Waiting on criterion 2, which is unrun, and on criterion 3, whose two renders include a
`fail` by this type's own definition.** Criterion 1 is cleared: 10 distinct sources on
2026-09-15, generated by `python3 scripts/validate.py --evidence` — four before the
157-image drop of 2026-09-11 and six from it. Until then this paragraph said *one more
distinct source*: the number was typed at four and the drop moved it without anyone opening
the file (ADR-084).
